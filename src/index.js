    ...
    function createListMarkup(data) {
        ...
        return data
            .map(({ name, flags }) =>
                `<li class="country-list__item" data-country='${name.common}'><img class="country-list__image" src="${flags.svg}" alt="${name.common}" height="40px" "/><p class="country-list__post">${name.common}</p></li>`
            )
            .join('');
        ...
        if (data.length === 1) {
            ...
            listCountryItem.forEach(item => {
                item.addEventListener('click', event => {
                    ...
                    infoCountry.innerHTML = createDataMarkup(data);
                    console.log('item clicked!', currentItem, event);
                    cleanListCountry();
                });
            });
        };
